...
#------------------------------------------------------------
# Get all the listings from the database, package them up,
...
# ------------------------------------------------------------
# Get the top 5 most expensive listings from the database
...
# in the database. 
@listings.route('/listing', methods=['POST'])
def add_new_product():
    ...
    query = f'''
        INSERT INTO listings (product_name,
                              description,
                              category, 
                              list_price)
    ...
    # query = 'insert into listings (product_name, description, category, list_price) values ("'
    # query += name + '", "'
    # query += description + '", "'
    # query += category + '", '
    # query += str(price) + ')'
...
@listings.route('/product', methods = ['PUT'])
def update_product():
